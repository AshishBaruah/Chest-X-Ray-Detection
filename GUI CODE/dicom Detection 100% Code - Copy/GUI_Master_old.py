...
img=ImageTk.PhotoImage(Image.open("D1.jpg"))

img2=ImageTk.PhotoImage(Image.open("D5.jpg"))

img3=ImageTk.PhotoImage(Image.open("D3.jpg"))
...
lbl = tk.Label(root, text=" DICOM IMAGE PROCESSING USING DEEP LEARNING FOR CHEST X-RAYS", font=('times', 20,' bold '), height=1, width=95,bg="violet Red",fg="Black")
lbl.place(x=0, y=0)
...
def convert_str_to_tuple(tup):#multiple items single variable madhe store karu shakto
...
def openimage():
    ...
    global fn
    fileName = askopenfilename(initialdir='D:/project code/100%code  Dicom Detection', title='Select image for Aanalysis ',
                               filetypes=[("all files", "*.*")])
    IMAGE_SIZE=200
    imgpath = fileName
    fn = fileName
